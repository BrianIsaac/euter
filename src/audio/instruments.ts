...
/** Every sample object a remote instrument needs before smplr can sound its full range. */
...
export async function loadInstrument(
  ...
  if (!entry.bundled) {
    // smplr's loaders log failed buffers and resolve anyway, so every expected object is checked
    // before the loader sees the pack. Otherwise an absent or partial upload can leave a silent
    // range with no notice.
